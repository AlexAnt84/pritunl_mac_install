...
homebrew_install ()
{
 echo "Установка Homebrew началась. Это займет некоторое время..."
 NONINTERACTIVE=1 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" 1> /dev/null
 if [[ "$?" != "0" ]] 
 then 
...
find $HMBR_PREFIX/bin/brew > /dev/null
if [[ $? != "0" ]] 
then
  homebrew_install
else
